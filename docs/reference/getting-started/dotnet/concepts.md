...
那么下面这张草图应该这样读：**下面**那个 foo Pad 对应赋值号的左边（`foo =`），**上面**那个 foo Pad 对应被初始化的变量（`var foo = 1`）。也就是：

```
foo（下面的 Pad） = foo（上面的 Pad） + 1;
```

*（上游此处引用的图片在其仓库中已缺失：示意从 IOBox 连到 foo Pad 的那条线是白色的，表示它被指派给了构造器）*
