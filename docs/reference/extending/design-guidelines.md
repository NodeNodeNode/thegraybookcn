...
### Create vs. (Join) vs. From... / Create、(Join) 与 From… {#create-vs-join-vs-from}
...
#### Join and Split / Join 与 Split {#join-and-split}
...
#### From.. and To.. / From… 与 To… {#from-and-to}
...
#### ..To.. Converters / ..To.. 转换器 {#to-converters}
...
#### Order of Pins / 针脚顺序 {#order-of-pins}
...
## Standard Datatypes / 标准数据类型 {#standard-datatypes}
...
## Standard Units / 标准单位 {#standard-units}

* 颜色分量（红、绿、蓝、alpha、色相、饱和度、明度）取值范围 0 到 1
* 角度以周期为单位（0 到 1，逆时针）

## Patterns / 若干模式 {#patterns}

### Dynamic Pin Counts / 动态针脚数 {#dynamic-pin-counts}
...
### Adaptive Nodes / 自适应节点 {#adaptive-nodes}
...
#### Adaptive Definition / 自适应定义 {#adaptive-definition}

建一个 Operation，放进顶层的 `Adaptive` 目录，加上输入输出针脚，名字随你喜欢。个别针脚可以做类型标注，但至少留一个泛化的，否则就没法为这个定义提供不同实现。

#### Adaptive Implementations / 自适应实现 {#adaptive-implementations}

在任何其他目录里建一个 Operation，用同样的签名，并以非泛化的方式实现它 —— 也就是说这一次所有输入输出都需要有推断出的或标注好的数据类型。

创建多个实现（针对不同数据类型）时，记得把这些 Operation 放进不同的目录。

#### Replace an adaptive node with a specific implementation / 把自适应节点替换成某个具体实现 {#replace-an-adaptive-node-with-a-specific-implementation}

有时你想确保编译器为某个自适应节点用上指定的实现。做法是：先通过节点浏览器摆下节点，然后双击，主面板里会列出所有可用实现，挑一个即可。

### Process Nodes / 过程节点 {#process-nodes-1}

#### Reset Inputs / Reset 输入 {#reset-inputs}

Reset 永远优先于其他输入（在过程浏览器里排最下）

* 例如：FlipFlop

### Nodes that operate async / 异步工作的节点 {#nodes-that-operate-async}
...
### Caching outputs / 缓存输出 {#caching-outputs}

什么时候该做、什么时候不该做。

### Resolving relative paths from within a node / 在节点内部解析相对路径 {#resolving-relative-paths-from-within-a-node}

目前还做不到。眼下你得用绝对文件名。

### Saving Data / 保存数据 {#saving-data}
...
### Resource Providers / 资源提供者 {#resource-providers}
...
### Restore Methods / Restore 方法 {#restore-methods}
