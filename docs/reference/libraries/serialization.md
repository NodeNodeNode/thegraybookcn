...
这种情况下，你可以用现成的运行时序列化器 —— 它们能直接序列化大多数数据类型，不需要你为序列化另建一套数据结构。这样又快又省事，代价是你对格式没有任何控制权，因而在体积上可能有些开销：序列化过程可能会把一些你根本不需要序列化的数据也算进去。
...
:::note
这些节点生成的序列化格式是「易变的」—— 也就是说它在 VL 的不同版本之间可能并不兼容。

另外：如果一份 JSON 或 XML 不是由对应的 Serialize 节点生成的，而其中各个属性又没有按字母顺序排列，那么反序列化会失败！这是 FsPickler 的一个怪癖，因此我们大概在某个时候得把它换掉。在那之前，碰上这类情况你不妨考虑改用 [Json.NET](https://www.newtonsoft.com/json) 库里的节点。
:::

如果你想更多了解这些节点内部是怎么工作的，去看它们所基于的 [FSPickler（英文）](https://mbraceproject.github.io/FsPickler/)库的文档。
