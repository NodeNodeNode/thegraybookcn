...
| 启用 VL.Stride 的 GPU 调试，见 [GPU 调试（英文）](https://thegraybook.vvvv.org/reference/libraries/3d/gpu-debugging.html) | `--renderdoc` 和 `--debug-gpu` |
...
下面这些参数对 vvvv.exe 和 [vvvvc.exe（英文）](https://thegraybook.vvvv.org/reference/hde/exporting.html#the-commandline-compiler)都适用：

| 作用 | 参数 |
|---|---|
| 替换 NuGet 的默认全局位置 | `--nuget-path` + 目录路径 |
| VL 查找包时使用的包仓库列表。在[参与节点库开发（英文）](https://thegraybook.vvvv.org/reference/extending/contributing.html)时最有用 | `--package-repositories` + 分号分隔的包仓库目录列表 |
...
下面这些参数只对 [vvvvc.exe（英文）](https://thegraybook.vvvv.org/reference/hde/exporting.html#the-commandline-compiler)适用（*斜体*为默认值）：
...
| 导出的程序里资源如何被引用，见 [Asset behavior（英文）](https://thegraybook.vvvv.org/reference/hde/exporting.html#asset-behavior) | `--asset-behavior` + RelativeToDocument \| *RelativeToOutput* |
