...
单说「运算器」这个词有时候会含糊：它既可能指*一个运算器的定义*，也可能指*这个定义的一次应用* —— 而后者其实就是我们平时说的 *Node*（节点）。在这一章里，「运算器」一律是「运算器定义」的简称。
...
一个数据类型可以有很多运算器，最常见的是至少有一个 `Create` 和一个 `Update`。为了在 *Patch*（草图）里区分多个成员运算器，VL 用颜色来标记 *Pin*（针脚）和 *Link*（连线）。有三个颜色是保留的：
...
通常从输入或输出针脚开始指派比较合理。注意：**指派会自动在整个草图里传播**，只在 *Pad*（数据板）和 *Process Node*（过程节点）那里停下 —— 这两者有点像运算器之间的桥：一个运算器写进去的值存在那里，另一个运算器可以取出来用。
