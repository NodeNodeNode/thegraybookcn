...
- 列出嵌套的元素，比如[类型草图定义](patches.md#datatype-patches)、[静态运算器定义](operations.md#creating-a-static-operation)和[目录（英文）](https://thegraybook.vvvv.org/reference/language/categories.html)
...
- 可以配置 [Process](patches.md#process) 定义
  ...
  - 启用／停用 State 输出
  - 通过勾选框决定哪些运算器属于这个 Process
  - 上下拖动运算器，决定它们在 Process 里的执行顺序
- 列出嵌套的元素，比如[类型草图定义](patches.md#datatype-patches)和[静态运算器定义](operations.md#creating-a-static-operation)
