...
## 运算器应用 {#operation-applications}

一个运算器应用，应用的是一个运算器。

运算器是很原始的东西。它是一次临时的计算：需要数据，产出数据。它可能带副作用，比如写文件，但通常它就是「基于数据算出数据」。

你通过它的输入针脚把数据喂给运算器应用，它算完把结果交给你。把它想成某种易逝的、暂时的东西，像一场化学反应。

**它的输出不依赖于此前对这个运算器的调用。** 它根本不知道自己以前被调用过。
...
拿它们的样子和上面的运算器应用比一比。差别很小，但日后当你要理清一张草图时，这点差别可能帮上忙。
