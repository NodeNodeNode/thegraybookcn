...
## Recap of immutable data and how to produce new values / 回顾：不可变数据，以及如何产生新值 {#recap-of-immutable-data-and-how-to-produce-new-values}
...
## Is the data flowing over a link changing over time? / 连线上流过的数据会随时间变化吗 {#is-the-data-flowing-over-a-link-changing-over-time}
...
## Mutable Object "Snapshots" / 可变对象的「快照」 {#mutable-object-snapshots}
...
## Immutable Object Snapshots share an Identity / 不可变对象的快照共享同一个身份 {#immutable-object-snapshots-share-an-identity}

关于 Record 的快照还有一点有意思：往 Spread 里加一项、或给 Record 设一个参数而产生新快照时，新对象与被克隆的那个共享同一个「身份」。于是你能看出哪些快照是一家人。

## Changed nodes are everywhere / Changed 节点无处不在 {#changed-nodes-are-everywhere}
